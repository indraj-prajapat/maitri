...
def save_mappings2():
    ...
    try:
        ...
        # Extract unique source and target messages from the payload (filter out NONE)
        def _clean(msgs):
            cleaned = []
            for x in msgs:
                if x is None:
                    continue
                if isinstance(x, str) and x.strip().upper() == "NONE":
                    continue
                cleaned.append(x)
            return cleaned
        source_massages = sorted(list(set(_clean(m['sourceMassage'] for m in approved_mappings))))
        target_massages = sorted(list(set(_clean(m['targetMassage'] for m in approved_mappings))))
        ...
        if main_mapping:
            ...
            main_mapping.source_massages = sorted(list(set(_clean(main_mapping.source_massages + source_massages))))
            main_mapping.target_massages = sorted(list(set(_clean(main_mapping.target_massages + target_massages))))
        else:
