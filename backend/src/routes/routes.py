...
def create_mapping():
    ...
    required = {
        "sourceCountry", "sourceDomain", "sourceSystem", "sourceLocation",
        "targetCountry", "targetDomain", "targetSystem", "targetLocation",
    ...
    with SessionLocal.begin() as db:
        ...
        # Natural-key lookup first (independent of id)
        nk = {
            "source_country": payload["sourceCountry"].strip() if isinstance(payload["sourceCountry"], str) else payload["sourceCountry"],
            "source_domain":  payload["sourceDomain"].strip()  if isinstance(payload["sourceDomain"], str)  else payload["sourceDomain"],
            "source_system":  payload["sourceSystem"].strip()  if isinstance(payload["sourceSystem"], str)  else payload["sourceSystem"],
            "source_location": (payload.get("sourceLocation").strip() if isinstance(payload.get("sourceLocation"), str) else payload.get("sourceLocation")),
            "target_country": payload["targetCountry"].strip() if isinstance(payload["targetCountry"], str) else payload["targetCountry"],
            "target_domain":  payload["targetDomain"].strip()  if isinstance(payload["targetDomain"], str)  else payload["targetDomain"],
            "target_system":  payload["targetSystem"].strip()  if isinstance(payload["targetSystem"], str)  else payload["targetSystem"],
            "target_location": (payload.get("targetLocation").strip() if isinstance(payload.get("targetLocation"), str) else payload.get("targetLocation")),
        }
        existing_metas = db.query(Metadata).filter_by(**nk).all()
        if not existing_metas and meta_id:
            existing_by_id = db.query(Metadata).filter_by(id=meta_id).first()
            existing_metas = [existing_by_id] if existing_by_id else []
        ...
        if existing_metas:
            ...
            for pair in payload["approvedMappings"]:
                ...
                incoming_normalized.append(normalize_mapping(pair, res))

            for candidate in existing_metas:
                existing_mappings = db.query(Mapping).filter_by(metadata_id=candidate.id).all()
                existing_normalized = [
                    {
